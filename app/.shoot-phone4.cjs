...
app.whenReady().then(async () => {
  try {
    ...
    const cdpShot = async (name) => {
      // Poke a repaint so the capture never waits on a frame that isn't coming.
      await js(`document.body.style.opacity = '0.999'; await new Promise(r => requestAnimationFrame(r)); document.body.style.opacity = '';`);
      await sleep(150);
